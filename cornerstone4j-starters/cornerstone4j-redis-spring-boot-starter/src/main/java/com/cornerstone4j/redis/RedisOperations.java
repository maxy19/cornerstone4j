package com.cornerstone4j.redis;
...
public class RedisOperations {
    ...
    public <V> List<V> multiGet(Collection<String> keys) {
        return isNotEmpty(keys) ? redisTemplate.opsForValue().multiGet(keys) : new ArrayList<>();
    }
    ...
    public long incr(String key, long delta) {
        if (delta < 0) {
            throw new RedisException("The increasing factor [{}] must be greater than 0");
        }
    ...
    public long decr(String key, long delta) {
        if (delta < 0) {
            throw new RedisException("The decreasing factor [{}] must be greater than 0.");
        }
